...
def index():
    ...
    # Statistik
    top_category = pd.crosstab(index=df2.Category, columns=df2.App.count()).unstack().reset_index().rename(columns={0: "Jumlah"})
    top_category = top_category.drop('col_0',axis=1)
    top_category = top_category.set_index('Category', drop = True)
    top_category.sort_values('Jumlah',ascending=False).head(5)

    rev_table = df2[['Category','App','Reviews','Rating']].sort_values(by='Reviews',ascending=False).reset_index(drop=True).head(10).drop_duplicates('App').reset_index(drop=True)
